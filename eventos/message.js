...
module.exports = class MessageEvent {
  ...
  async run(message) {
    ...
    try {
      ...
      embed.setAuthor(message.author.tag, message.author.displayAvatarURL({ size: 2048 })).setTimestamp();
      if (message.guild) {
        ...
        if (message.content.toLowerCase().startsWith('prefijo')) {
          embed
            ...
            .setDescription('¡¿Olvidaste mi prefijo?! No importa, aquí te lo digo; `' + prefix + '`');
      ...
      data.user = user;
      let dmserverprefix = message.guild ? data.guild.prefix : client.config.misc.prefix;
