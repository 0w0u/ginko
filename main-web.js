...
module.exports = class Web {
  ...
  start() {
    ...
    app.get("/invite", (req, res) => {
      res.redirect("https://discordapp.com/oauth2/authorize?client_id=621097720781996072&scope=bot&permissions=2146958839");
    });
    ...
    app.post("/wbl/dbl", async (req, res) => {
      if (!req.headers.authorization || req.headers.authorization !== tokens.votes.password)
        return res.json({
          status: 400,
      ...
      let usuario = (await this.client.users.get(user)) || (await this.client.fetchUser(user));
