...
module.exports = class Informative extends Command {
  constructor(client) {
    super(client, {
      name: 'ayuda',
      description:
        'Obtén información sobre el bot, o sobre un comando específico, o la lista de comandos',
      usage: prefix => '`' + prefix + 'ayuda [comando | lista]`',
      examples: prefix =>
        '`' + prefix + 'ayuda eval`, `' + prefix + 'ayuda lista`',
      enabled: true,
      ownerOnly: false,
      guildOnly: false,
      nsfwOnly: false,
    ...
    try {
      ...
      if (!args[0]) {
        embed
          ...
          .setDescription(
            'Éste es el comando de ayuda, y ésta es información que debes saber sobre mí\nUsa `' +
              message.dmguildprefix +
              'ayuda lista` para ver mi lista de comandos'
          )
          .setThumbnail(message.client.user.displayAvatarURL({ size: 2048 }))
          .addField(
            '¡Advertencias!',
            'No uses literalmente `< >`, `[ ]`, `< | >` a la hora de ejecutar un comando,\nestos son representados como; `Argumento obligatorio`, `Argumento opcional` y `Opciones de argumentos` respectivamente'
          )
          .addField(
            '¡Enlaces útiles!',
            '[`Servidor de soporte   `](' +
              message.misc.others.support +
              ')  [`Enlace de invitación   `](' +
              message.misc.others.invite +
              ')\n[`Código abierto        `](' +
              message.misc.others.glitch +
              ')  [`Patreon                `](' +
              message.misc.others.donate +
              ')'
          )
          .setImage(
            'https://media.discordapp.net/attachments/632984081709269015/657037947895545866/ginkobanner.png'
          )
          .setFooter(
            (message.guild ? message.guild.name : this.client.user.username) +
              ' | Versión: ' +
              version,
            message.guild
        ...
        args[0].toLowerCase() === 'lista' ||
        args[0].toLowerCase() === 'comandos'
      ) {
        let or = {
          INFORMATIVOS: 10,
          ÚTILES: 11,
        ...
        categorias.forEach(cat => {
          ...
          embed.addField(
            ...
            '`' + commandsize.map(cmd => cmd.help.name).join('`, `') + '`'
          );
        });
        message.channel.send({ embed });
        return;
        ...
        if (
          ...
          let ayuda =
            ...
            this.client.commands.get(aliases.get(args[0].toLowerCase()));
          embed
            .setColor(message.colors.ginko)
            .setTitle(
              message.defaults.ginkoun +
                ...
                ayuda.help.category +
                '`: ayuda detallada de `' +
                (ayuda.help.name[0].toUpperCase() + ayuda.help.name.slice(1)) +
                '`'
            )
            .addField(
              'Descripción',
              ayuda.help.description !== ' '
                ? ayuda.help.description
                : 'No se proporcionó una descripción para este comando'
            )
