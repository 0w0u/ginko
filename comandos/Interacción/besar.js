...
module.exports = class Help extends Command {
  constructor(client) {
    super(client, {
      ...
      usage: prefix => "`" + prefix + "besar <usuario>`",
      examples: prefix => "`" + prefix + "besar mon`",
      enabled: true,
      ownerOnly: false,
      guildOnly: true,
      nsfwOnly: false,
  ...
  async run(message, args, data, embed) {
    try {
      ...
      async function o(m) {
        let img = await client.tools.sfw.kiss();
        embed
          .setColor(message.colors.ginko)
          .setTitle(message.defaults.ginkoun + message.member.displayName + ' está besando a ' + m.displayName)
          .setDescription('Awwwww... <:giMonGasm:623336877700808709>')
          .setImage(img.url);
        message.channel.send({ embed });
      }
    } catch (e) {
