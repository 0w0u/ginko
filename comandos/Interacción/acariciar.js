...
module.exports = class Interaction extends Command {
  constructor(client) {
    super(client, {
      ...
      usage: prefix => '`' + prefix + 'acariciar <usuario>`',
      examples: prefix => '`' + prefix + 'acariciar mon`',
      enabled: true,
      ownerOnly: false,
      guildOnly: true,
      nsfwOnly: false,
  ...
  async run(message, args, data, embed) {
    try {
      ...
      async function o(m) {
        let img = await client.tools.sfw.pat();
        embed
          .setColor(message.colors.ginko)
          .setTitle(message.defaults.ginkoun + message.member.displayName + ' acaricia a ' + m.displayName)
          .setDescription('Awwwww... <:giMonUwU:623336878094942228>')
          .setImage(img.url);
        message.channel.send({ embed });
      }
    } catch (e) {
