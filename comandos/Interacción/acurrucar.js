...
module.exports = class Interaction extends Command {
  constructor(client) {
    super(client, {
      ...
      usage: prefix => '`' + prefix + 'acurrucar <usuario>`',
      examples: prefix => '`' + prefix + 'acurrucar mon`',
      enabled: true,
      ownerOnly: false,
      guildOnly: true,
      nsfwOnly: false,
    ...
    try {
      ...
      async function o(m) {
        ...
        embed
          ...
          .setTitle(message.defaults.ginkoun + message.member.displayName + ' se acurruca con ' + m.displayName)
          .setDescription('Awww... <:giMonShy:623336878145273864>')
          .setImage(img.url);
        message.channel.send({ embed });
      }
    } catch (e) {
