...
module.exports = class Interaction extends Command {
  constructor(client) {
    super(client, {
      ...
      usage: prefix => '`' + prefix + 'abrazar <usuario>`',
      examples: prefix => '`' + prefix + 'abrazar mon`',
      enabled: true,
      ownerOnly: false,
      guildOnly: true,
      nsfwOnly: false,
  ...
  async run(message, args, data, embed) {
    try {
      ...
      async function o(m) {
        let img = await client.tools.sfw.hug();
        embed
          .setColor(message.colors.ginko)
          .setTitle(message.defaults.ginkoun + message.member.displayName + ' abraza a ' + m.displayName)
