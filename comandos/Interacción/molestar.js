...
module.exports = class Help extends Command {
  constructor(client) {
    super(client, {
      ...
      usage: prefix => "`" + prefix + "molestar <usuario>`",
      examples: prefix => "`" + prefix + "molestar mon`",
      enabled: true,
      ownerOnly: false,
      guildOnly: true,
      nsfwOnly: false,
  ...
  async run(message, args, data, embed) {
    try {
      ...
      async function o(m) {
        let img = await client.tools.sfw.poke();
        embed
          .setColor(message.colors.ginko)
          .setTitle(message.defaults.ginkoun + message.member.displayName + ' está molestando a ' + m.displayName)
          .setDescription('¡Qué molestooo! <:giMonNoSleep:623336878074101816>')
          .setImage(img.url);
        message.channel.send({ embed });
      }
    } catch (e) {
