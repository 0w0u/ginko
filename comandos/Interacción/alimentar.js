...
module.exports = class FeedCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      usage: prefix => '`' + prefix + 'alimentar <usuario>`',
      examples: prefix => '`' + prefix + 'alimentar mon`',
      enabled: true,
      ownerOnly: false,
      guildOnly: true,
      nsfwOnly: false,
  ...
  async run(message, args, data, embed) {
    try {
      ...
      async function o(m) {
        let img = await client.tools.sfw.feed();
        embed
          .setColor(message.colors.ginko)
          .setTitle(message.defaults.ginkoun + message.member.displayName + ' está dándole de comer a ' + m.displayName)
          .setDescription('Nom nom <:giMonPopcorn:623336878200062002>')
          .setImage(img.url);
        message.channel.send({ embed });
      }
    } catch (e) {
