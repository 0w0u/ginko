...
module.exports = class Interaction extends Command {
  constructor(client) {
    super(client, {
      ...
      usage: prefix => '`' + prefix + 'cosquillas <usuario>`',
      examples: prefix => '`' + prefix + 'cosquillas mon`',
      enabled: true,
      ownerOnly: false,
      guildOnly: true,
      nsfwOnly: false,
  ...
  async run(message, args, data, embed) {
    try {
      ...
      async function o(m) {
        let img = await client.tools.sfw.tickle();
        embed
          .setColor(message.colors.ginko)
          .setTitle(message.defaults.ginkoun + message.member.displayName + ' está cosquilleando a ' + m.displayName)
          .setDescription('¡Qué divertido! <:giMonLul:623336878044741664>')
          .setImage(img.url);
        message.channel.send({ embed });
      }
    } catch (e) {
