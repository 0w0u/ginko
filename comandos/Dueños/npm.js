  ...
  async run(message, args, data, embed) {
    try {
      ...
      if (!args[0]) {
        ...
        return;
      } else if (args[0].toLowerCase() === 'add' || args[0].toLowerCase() === 'añadir') {
        if (!args[1]) {
          embed
            .setColor(message.colors.red)
            .setTitle(message.defaults.noargs)
          ...
          cmd.exec('pnpm install ' + args[1] + ' --save', function (err, stdout, stderr) {
            if (err) {
              embed
                .setColor(message.colors.red)
                .setTitle(message.default.error)
                .setDescription('Ha ocurrido un error y no se pudo instalar el paquete');
              msg.edit({ embed });
              return;
            } else {
              embed
          ...
          cmd.exec('pnpm uninstall ' + args[1] + ' --save', function (err, stdout, stderr) {
            if (err) {
              embed
                ...
                .setDescription('Ha ocurrido un error y no se pudo desinstalar el paquetr');
              msg.edit({ embed });
              return;
            } else {
              embed
