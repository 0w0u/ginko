  ...
  async run(message, args, data, embed) {
    try {
      let author = message.author,
        ...
        dguild,
        dmember;
      guild ? (dguild = await client.findOrCreateGuild({ id: guild.id })) : '¡No estás en una guild!';
      member
      ...
      try {
        ...
        if (err.toString().includes(client.config.tokens.bot || client.config.tokens.mongo)) err = err.toString().replace(client.config.tokens.bot || client.config.tokens.mongo, 'T0K3N');
